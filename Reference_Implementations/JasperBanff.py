...
EP_MAX_STEPS = 2000  # Step limitation in an episode
...
def init(env, env_name):
    ...
    action spaces
    """
    global replay_buffer, epsilon, iscontinuous,state_in_, q_next, default_env_name
    replay_buffer = []
    ...
    try:
        ...
        for i in range(ACTION_Dim):
            action_dict[i] = low + (high - low)/ACTION_Dim * i
        action_dict[ACTION_Dim] = env.action_space.high[0]
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    """
    def build_layers(state_in, state_dim, hidden_nodes, action_dim, c_names, w_initializer, b_initializer):
        with tf.variable_scope('l1'):
            #w1a = tf.get_variable('w1a',[state_dim,hidden_nodes],initializer=w_initializer, collections=c_names)
            #b1a = tf.get_variable('b1a',[1,hidden_nodes], initializer=b_initializer, collections=c_names)
            #l1a = tf.nn.relu(tf.matmul(state_in,w1a) + b1a) # shape = (hid,1)

            w1 = tf.get_variable('w1',[state_dim,hidden_nodes],initializer=w_initializer, collections=c_names)
            b1 = tf.get_variable('b1',[1,hidden_nodes], initializer=b_initializer, collections=c_names)
            l1 = tf.nn.relu(tf.matmul(state_in,w1) + b1) # shape = (hid,1)

        with tf.variable_scope('Value'):
            w2 = tf.get_variable('w2',[hidden_nodes,1],initializer=w_initializer, collections=c_names)
            b2 = tf.get_variable('b2', [1, 1], initializer=b_initializer, collections=c_names)
            Value = tf.matmul(l1, w2) + b2

        with tf.variable_scope('Advantage'):
            w2 = tf.get_variable('w2', [hidden_nodes, action_dim], initializer=w_initializer, collections=c_names)
            b2 = tf.get_variable('b2', [1, action_dim], initializer=b_initializer, collections=c_names)
            Advantage = tf.matmul(l1, w2) + b2
    ...
    state_in = tf.placeholder("float", [None, state_dim], name = 's')
    ...
    with tf.variable_scope('eval_net'):
        c_names, n_l1, w_initializer, b_initializer = \
            ['eval_net_params', tf.GraphKeys.GLOBAL_VARIABLES], hidden_nodes, \
            tf.random_normal_initializer(0., 0.3), tf.constant_initializer(0.1)
        q_values = build_layers(state_in, state_dim, hidden_nodes,action_dim, c_names, w_initializer, b_initializer)
    # q_eval
    ...
    state_in_ = tf.placeholder("float", [None, state_dim], name='s_')

    global q_next
    with tf.variable_scope('target_net'):
        
        c_names = ['target_net_params', tf.GraphKeys.GLOBAL_VARIABLES]
        q_next  = build_layers(state_in_, state_dim, hidden_nodes,action_dim, c_names, w_initializer, b_initializer)
...
def init_session():
    global session, writer,t_params,e_params,replace_target_op
...
def get_env_action(action, observation):
    ...
    if iscontinuous == True:
        ...
        return np.array([action_dict[action]])
    return action
...
def get_train_batch(q_values, state_in, replay_buffer):
    ...
    global q_next, state_in_

    target_batch = []
    q_next_1 = q_next.eval(feed_dict={
        state_in_: next_state_batch
    })
    for i in range(0, BATCH_SIZE):
        ...
        if sample_is_done:
            ...
            target_val = reward_batch[i] + GAMMA * np.max(q_next_1[i])
...
def qtrain(env, state_dim, action_dim,
    ...
    for episode in range(num_episodes):
        ...
        for step in range(ep_max_steps):
            total_steps += 1
            if total_steps > 0 and total_steps % replace_target_iter ==0:
            ...
            action = get_action(state, state_in, q_values, epsilon, test_mode,
                                action_dim)
            env_action = get_env_action(action,state)
